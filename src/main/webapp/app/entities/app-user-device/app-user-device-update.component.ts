      ...
      if (this.appUserDevice.id) {
        this.appUserDeviceService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('shipperfinderservice3App.appUserDevice.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.appUserDeviceService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('shipperfinderservice3App.appUserDevice.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
