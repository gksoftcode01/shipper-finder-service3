      ...
      if (this.itemType.id) {
        this.itemTypeService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('shipperfinderservice3App.itemType.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.itemTypeService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('shipperfinderservice3App.itemType.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
