      ...
      if (this.userRate.id) {
        this.userRateService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('shipperfinderservice3App.userRate.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.userRateService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('shipperfinderservice3App.userRate.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
