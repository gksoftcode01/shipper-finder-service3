      ...
      if (this.showNumberHistory.id) {
        this.showNumberHistoryService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('shipperfinderservice3App.showNumberHistory.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.showNumberHistoryService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('shipperfinderservice3App.showNumberHistory.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
