      ...
      if (this.subscribeTypeDetail.id) {
        this.subscribeTypeDetailService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('shipperfinderservice3App.subscribeTypeDetail.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.subscribeTypeDetailService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('shipperfinderservice3App.subscribeTypeDetail.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
